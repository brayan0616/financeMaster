...
class GeometricoPage extends StatefulWidget {
  ...
  @override
  State<GeometricoPage> createState() => _TiempoCompuestoPageState();
}

enum TipoGradiente { creciente, decreciente }

class _TiempoCompuestoPageState extends State<GeometricoPage> {
  ...
  String obtenerFormulaValorPresente() {
    return GeometricoCalculator.obtenerFormulaValorPresente(tipoGradiente.name);
  }

  String obtenerFormulaValorFuturo() {
    return GeometricoCalculator.obtenerFormulaValorFuturo(tipoGradiente.name);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('GEOMETRICO', style: TextStyle(color: colors.primary)),
      ),
                ...
                children: [                 
                  RichText(
                    ...
                    text: TextSpan(
                      children: <TextSpan>[
                        TextSpan(
                          text: "Geométrico", style: textStyle.titleSmall?.copyWith(fontWeight: FontWeight.bold, fontSize: 18),
                        ),
                        TextSpan(
                          text: " es un tipo de gradiente el cual cada pago es igual al del periodo anterior," 
                                " aumentando o dosminuyendo en un mismo porcentaje.", style: textStyle.titleSmall?.copyWith(fontWeight: FontWeight.w500, fontSize: 18),
                        ),
                        TextSpan(
